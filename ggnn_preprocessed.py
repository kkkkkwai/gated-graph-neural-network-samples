...
class ProgramModel(object):
    ...
    def __init__(self, args):
        ...
        # Specify data path:
        self.project_name = 'benchmarkdotnet'
        ...
        self.data_path = os.path.join(self.data_path, self.project_name)

        self.type_path = os.path.join(self.data_path, 'benchmarkdotnet-typehierarchy.json')
        self.type_hierarchy = []
        ...
        self.train_data_path = os.path.join(self.data_path, 'graphs-train')
        self.valid_data_path = os.path.join(self.data_path, 'graphs-valid')
        self.test_data_path = os.path.join(self.data_path, 'graphs-test')
    ...
    def load_data(self, dir_path, filename):
    ...
    def train(self):
        ...
        with self.graph.as_default():
            if self.args.get('--restore') is not None:
                ...
                for idx, filename in enumerate(os.listdir(self.valid_data_path)):
                    if not filename.endswith('json'):
                        self.valid_data = self.load_data(self.valid_data_path, filename)
                        _, valid_acc, _, num_graphs = self.run_epoch("Resumed (validation)", self.valid_data, False)
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                ...
                for idx, filename in enumerate(os.listdir(self.train_data_path)):
                    if filename.endswith('json'):
                        self.train_data = self.load_data(self.train_data_path, filename)
                        train_loss, train_acc, train_time, num_graphs = self.run_epoch("epoch %i (training)" % epoch,
                                                                                       filename,
                                                                                       self.train_data,
                                                                                       True)
                ...
                for idx, filename in enumerate(os.listdir(self.valid_data_path)):
                    if filename.endswith('json'):
                        self.valid_data = self.load_data(self.valid_data_path, filename)
                        valid_loss, valid_acc, valid_time, num_graphs = self.run_epoch("epoch %i (validation)" % epoch,
                                                                                       filename,
                                                                                       self.valid_data,
                                                                                       False)
    ...
    def plot_chart(self, epoch:int, train_losses_series, train_accs_series, valid_losses_series, valid_accs_series):
        plt.figure()
